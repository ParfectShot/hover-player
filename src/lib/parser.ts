...
const checkIfElementIsInIgnoreList = (element: HTMLElement) => {
  return IGNORE_LIST.includes(element.tagName)
}

const checkIfElementTextNodeIsNotEmpty = (element: HTMLElement) => {
  return Boolean(element.textContent?.trim().length)
}

const checkIfElementIsTopLevel = (element: HTMLElement) => {
  if (!checkIfElementTextNodeIsNotEmpty(element)) {
    return false;
  }
  if (checkIfElementIsInIgnoreList(element)) {
    return false
  }
  ...
  const checkChildren = (node: Node): boolean => {
    if (!node) {return true}
    if (node.nodeType === Node.TEXT_NODE) {
      return checkIfElementIsTopLevel(node as HTMLElement);
    }
    ...
    return checkChildren(elementNode.children[0]);
  };
  
  return checkChildren(element.children[0])
}
...
export function getTopLevelReadableElementsOnPage(): HTMLElement[] {
  ...
  function recursivelyCheckElement(element: HTMLElement) {
    if (checkIfElementIsTopLevel(element)) {
      topLevelReadableElements.push(element);
      return;
    }
    if (element.children.length > 0) {
      for (let i = 0; i < element.children.length; i++) {
        recursivelyCheckElement(element.children[i] as HTMLElement);
      }
    }
  }
  recursivelyCheckElement(body);

...
export default function HoverPlayer() {
  const lastHoveredElement = useRef<HoveredElementInfo | null>(null);
  const readableElements = useMemo(
    () => getTopLevelReadableElementsOnPage(),
    [],
  );
  const hoveredElement = useHoveredParagraphCoordinate(readableElements);
  const playButtonAnchor = hoveredElement || lastHoveredElement.current;

  const handlePlay = () => {
    speechify(playButtonAnchor?.element!);
  };

  useEffect(() => {
    if (hoveredElement) lastHoveredElement.current = hoveredElement;
  }, [hoveredElement]);
    ...
    <>
      {playButtonAnchor && (
        <div
          ...
          style={{
            position: "absolute",
            top: playButtonAnchor.top,
            left: playButtonAnchor.left - 24,
            cursor: "pointer",
          }}
          onClick={handlePlay}
        >
